...
def main():
    ...
    qemu_cmd = [
        ...
        "512M",
        "-append",
        f"console=ttyS0 quiet panic=-1 -- {args.mode} {args.output_file.name}",
        "-nographic",
        "-no-reboot",
        "-virtfs",
        f"local,path={output_dir},mount_tag=output,security_model=none,id=output",
    ]
    ...
    try:
        with log_file_path.open("w") as log_file:
            subprocess.run(
                qemu_cmd, stdout=log_file, stderr=subprocess.STDOUT, check=True
            )
